...
public class IceCreamConeView extends JComponent {
	...
	public void paintCone(Graphics g) {
		...
		int[] xPoints = {this.getWidth()/2, this.getWidth()/2+CONE_WIDTH/2, this.getWidth()/2-CONE_WIDTH/2};
		int[] yPoints = {this.getHeight(), this.getHeight()-CONE_HEIGHT, this.getHeight()-CONE_HEIGHT};
	...
	public void paintScoop(Graphics g, int flavorIndex, int x, int y) {
		...
		g.drawOval(x, y, SCOOP_DIAMETER, SCOOP_DIAMETER);
	...
	private void paintScoops(Graphics g) {
		...
		int x = this.getWidth()/2-SCOOP_DIAMETER/2, y = this.getHeight()-CONE_HEIGHT+SCOOP_OVERLAP-SCOOP_DIAMETER;
