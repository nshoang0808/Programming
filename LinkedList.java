...
public class LinkedList<T> {
	...
	public void insertLast(T data) {
		// Create a node representing the current node
		LinkedListNode<T> currentNode = new LinkedListNode<T>();
		// Create another node
		LinkedListNode<T> newNode = new LinkedListNode<T>();
		// Set the data to the new node
		newNode.setData(data);
		// Assign the value of head to this node variable, so we don't loose
		// head value
		currentNode = head;
		...
		if (currentNode == null) {
			...
			// And if the next value is not null
	...
	public void deleteNext(LinkedListNode<T> currentNode) {
		...
		for (LinkedListNode<T> node = head; node != null; node = node.getNext()) {
			// If gets to the currentNode
			if (node == currentNode) {
				// If the one after one after the current node is not null
				if (currentNode.getNext().getNext() != null) {
					// Set temporary value to be equal to it
					temp = currentNode.getNext().getNext();
					// Set temp to be the next of current node
